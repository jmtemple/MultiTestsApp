...
class TraceShapeViewController: UIViewController, JotStrokeDelegate, UIGestureRecognizerDelegate {
    ...
    override func viewDidLoad() {
        ...
        fileWriter.write(text: "\(Utils.timeOnly()), Trace Shape Game Begins")
        fileWriter.write(text: "\(Utils.timeOnly()), FIRST SHAPE BEGIN")
        
        // setting the size of the circle and square for iPhone vs. iPad
        if (device == "iPhone") {
            traceView.radius = 300.0  - traceView.gutter
            demoView.radius = 300.0  - demoView.gutter
        } else {
        ...
        /*
        let alert = UIAlertController(title: "Instructions", message: "Trace the following shapes. Hit submit when you are done", preferredStyle: UIAlertControllerStyle.alert)
    ...
    @IBAction func submitButton(_ sender: AnyObject) {
        ...
        if (points.count > 0) { // make sure there are points to analyze
            ...
            if(!firstSubmit){
                ...
                fileWriter.write(text: "\(Utils.timeOnly()), Trace Shape Game End")
            }
